...
namespace _041er_BLJ.classes
{
    public class ProjectsController
    {
        ...
        internal static Dictionary<string, List<Project>> GetProjects()
        {
            ...
            foreach (string projectDirs in Directory.GetDirectories(HttpContext.Current.Server.MapPath("/") + ProjectsFolder))
            {
                ...
                foreach (string projectDir in Directory.GetDirectories(projectDirs))
                {
                    ...
                    p.Url = ProjectsController.ProjectsFolder + "/" + p.ProjectsDirectory + "/" + Path.GetFileName(projectDir) + "/";
